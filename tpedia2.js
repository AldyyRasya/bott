...
global.PayDisiniKey = 'c6cc1696fa25a665eda4584173757f93'
...
AnanthaGanz.readMessages([m.key]) 
 ...
 case 'setweb':{
    let ananthaNick = text.split(" ")[0];
        if (!ananthaNick) return reply("Mohon masukkan API key Nick yang valid, contoh: .setweb zannkey-NxpMXoIo93-d3ECg");
    const axios = require("axios");
    var config = {
        method: 'POST',
    ...
    axios(config)
        .then(function (response) {
            if (response.data.status == true) {
             ...
             sett("apiWeb", m.sender, ananthaNick)
...
AnanthaGanz.sendMessage(m.sender,{image: dot, caption: ccapt },  { quoted: m })
}, 1000);
