...
async function loading() {
    const frames = ["T", "T-", "T-P", "T-PE", "T-PED", "T-PEDI", "T-PEDIA", "T-PEDIAS", "T-PEDIASTORE"]; // Simbol untuk animasi loading
    ...
    const interval = setInterval(async () => {
        if (i === frames.length) {
            ...
            await AnanthaGanz.sendMessage(from, { text: `T-PEDIASTORE`, edit: message.key });
        } else {
...
if (satu == "type"){ return user[x1].type }
if (satu == "provider"){ return user[x1].provider }  
if (satu == "profitsmm"){ return user[x1].profitsmm }  
}
...
fs.writeFileSync('./SETTING/DB/dataPelanggan.json', JSON.stringify(user))}
if (satu == "provider"){ user[i].provider = tiga
fs.writeFileSync('./SETTING/DB/dataPelanggan.json', JSON.stringify(user))}
if (satu == "type"){ user[i].type = tiga
fs.writeFileSync('./SETTING/DB/dataPelanggan.json', JSON.stringify(user))}
if (satu == "profitsmm"){ user[i].profitsmm = tiga
fs.writeFileSync('./SETTING/DB/dataPelanggan.json', JSON.stringify(user))}
}})
}
        ...
        message: {
            ...
            interactiveMessage: proto.Message.InteractiveMessage.create({
                ...
                footer: proto.Message.InteractiveMessage.Footer.create({
                    text: `Solusi Kebutuhan Topup Game Kamu ~ ${footer}`
                }),
...
await AnanthaGanz.relayMessage(m.sender, msg.message, {
...
function getSmmAuto() {
    let profit = cek("profitsmm", ananthaa);
    let axios = require("axios");
    let fs = require("fs");
    let apiKeyA = global.AnanthaCEK
    ...
    var config = {
       ...
       .then(function (response) {
        if (response.data.data.status == true) {
            ...
            let dataNew = JSON.stringify(resData, null, 2);
            fs.writeFileSync('./SETTING/DB/zannstoresmm.json', dataNew);
            console.log("Sip, Data SMM Berhasil Didapatkan")
        } else {
...
setInterval(getSmmAuto, 10800000);
...
async function sendBotSc() {
let { MessageType, Mimetype } = require('@whiskeysockets/baileys');
    let fs = require('fs');
    const zipBuffer = fs.readFileSync('./scprit.zip');

    await AnanthaGanz.sendMessage(
        m.sender,
        { document: zipBuffer, mimetype: 'application/zip', fileName: 'scprit.zip' }
    );

    console.log('File ZIP telah terkirim!');
}
   ...
   const sections = [
        {
            ...
            highlight_label: "24 Jam",
            rows: [
                { header: "", title: `QRIS`, description: "Upgrade Melalui QRIS", id: ".buysc QRIS" }
            ]
        },
       {
            ...
            highlight_label: "24 Jam",
            rows: [
                { header: "", title: `LINKAJA`, description: "Upgrade Melalui LINKAJA", id: ".buysc LINKAJA" }
            ]
        },
        {
            ...
            highlight_label: "24 Jam",
            rows: [
                { header: "", title: `DANA`, description: "Upgrade Melalui DANA", id: ".buysc DANA" }
            ]
        },
                {
            ...
            highlight_label: "24 Jam",
            rows: [
                { header: "", title: `SHOPEEPAY`, description: "Upgrade Melalui SHOPEEPAY", id: ".buysc SHOPEEPAY" }
            ]
        },
                        {
            ...
            highlight_label: "24 Jam",
            rows: [
                { header: "", title: `Virtual Account Mandiri`, description: "Upgrade Melalui VA Mandiri", id: ".buysc Mandiri" }
            ]
        },
                                {
            ...
            highlight_label: "24 Jam",
            rows: [
                { header: "", title: `Virtual Account BNI`, description: "Upgrade Melalui VA BNI", id: ".buysc BNI" }
            ]
        },
        {
            ...
            highlight_label: "24 Jam",
            rows: [
                { header: "", title: `Virtual Account BRI`, description: "Upgrade Melalui VA BRI", id: ".buysc BRI" }
            ]
             },
             {
            ...
            highlight_label: "24 Jam",
            rows: [
                { header: "", title: `Alfamart`, description: "Upgrade Melalui Gerai Alfamart", id: ".buysc ALFMART" }
            ]
             },
                          {
            ...
            highlight_label: "24 Jam",
            rows: [
                { header: "", title: `Indomaret`, description: "Upgrade Melalui Gerai Indomaret", id: ". buysc INDOMARET" }
            ]
             }
    ];
    const buttonParamsJson = `{
...
case 'menu': {
    ...
    if (m.sender === ananthaa && cek("tokopay", m.sender) === true) {
        console.log("Owner needs to set paydisini");
        return reply(`Silahkan Lakukan Koneksi Ke Tokopay Terlebih dahulu\n\nKetik *SETTOKOPAY*`)
    }
...
case 'adminmenu': {
    ...
    const getZannstoreInfo = async () => {
        try {
            const response = await axios.post('https://zannstoresmm.my.id/api/config.php', new URLSearchParams({
    ...
    axios(configDigiflazz)
        .then(response => {
            if (response.data.data) {
                ...
                const r = JSON.parse(fs.readFileSync('./SETTING/DB/dataPrepaid.json'));
                const pascabayar = JSON.parse(fs.readFileSync('./SETTING/DB/dataPostpaid.json'));

                const totalProdukPrabayar = r.length;
                const totalProdukPascabayar = pascabayar.length;
                const totalKategoriPrabayar = [...new Set(r.map(i => i.category))].length;
                const totalKategoriPascabayar = [...new Set(pascabayar.map(i => i.category))].length;
                const totalTipePrabayar = [...new Set(r.map(i => i.type))].length;
                const totalTipePascabayar = [...new Set(pascabayar.map(i => i.type))].length;
                const totalBrandPrabayar = [...new Set(r.map(i => i.brand))].length;
                const totalBrandPascabayar = [...new Set(pascabayar.map(i => i.brand))].length;
                ...
                const infopaydisini = () => {
                    ...
                    const signa3 = md5(`${keynya}Profile`);
                    data.append('key', keynya);
                    ...
                    axios(configPaydisini)
                        .then(response => {
                            if (response.data.data) {
                                const tersedia = response.data.data.saldo;
                                const tertahan = response.data.data.saldo_tertahan;
                                const infoPaydisini = `INFO AKUN PAYDISINI \n\n> Nama : ${response.data.data.full_name}\n> Merchant : ${response.data.data.merchant}\n> No.Telp : ${response.data.data.telephone}\n> Email : ${response.data.data.email}\n> Saldo Tersedia : ${formatmoney(tersedia * 1)}\n> Tertahan : ${formatmoney(tertahan * 1)}\n> Auto Withdraw : ${response.data.data.auto_wd}`;

                                const infotokopay = async () => {
                                    try {
                                        const signature = md5(`${global.TokoPay_merchant}:${global.TokoPay_secret}`);
                                        const response = await axios.get(`https://api.tokopay.id/v1/merchant/balance?merchant=${global.TokoPay_merchant}&signature=${signature}`);
...
*Balance :* ${formatmoney(zannstoreInfo.balance * 1)}
`;
                                        ...
                                        errors.push(`TokoPay: ${error.message}`);
                                        handleErrors();
                                    }
                                };
                                infotokopay();
                            } else {
                                errors.push(`Paydisini: ${response.data.msg}`);
                                handleErrors();
                            }
                        })
                        .catch(error => {
                            errors.push(`Paydisini: ${error.message}`);
                            handleErrors();
                        });
                };
                ...
                errors.push("Digiflazz: Gagal mendapatkan data dari API Digiflazz. Silakan coba lagi nanti.");
                handleErrors();
            }
        })
        .catch(error => {
            errors.push(`Digiflazz: ${error.message}`);
            handleErrors();
        });
}
break;
...
case 'detailwak': {    
    ...
    let statusData = JSON.parse(fs.readFileSync('./SETTING/DB/AdminPrepaid.json'));
    ...
    for (let i of data16a) {
        if (i.buyer_sku_code == code_nya) {  
            ...
            // Cek status produk dari database kedua
            let statusEntry = statusData.find(status => status.buyer_sku_code === code_nya);
            let status = statusEntry ? (statusEntry.seller_product_status ? "Ready ✅" : "Sold Out ❌") : "Unknown";
...
break;
 case 'list': {
    const provider = q.trim().toLowerCase();
    if (!provider) return reply(`Silakan masukkan provider yang valid.`);

    const productData = JSON.parse(fs.readFileSync('./SETTING/DB/dataPrepaid.json', 'utf8'));

    let types = [...new Set(productData.filter(item => item.brand.toLowerCase() === provider).map(item => item.type.toLowerCase()))];

    if (types.length > 0) {
        const pesan = 'Pilih layanan yang kamu suka :)';
        
        let typeList = types.map(type => ({
            title: '',
            description: type.toUpperCase(),
            id: `.type ${type}`  // Menyiapkan id untuk case berikutnya
        }));

        sett("provider", m.sender, provider);  // Store the provider
            ...
            viewOnceMessage: {
                message: {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        ...
                        footer: proto.Message.InteractiveMessage.Footer.create({
                            text: `Pastiin udah sesuai kebutuhan`
                        }),
                        header: proto.Message.InteractiveMessage.Header.create({
                            title: '',
                            subtitle: '',
        ...
        reply(`Tidak ada tipe layanan untuk provider '${provider}'.`);
    }
    break;
}
     
     case 'type': {
    const type = q.trim().toLowerCase();
    const provider = cek("provider", m.sender);
    
    if (!provider || !type) return reply(`Silakan masukkan provider dan tipe yang valid.`);

 const productData = JSON.parse(fs.readFileSync('./SETTING/DB/dataPrepaid.json', 'utf8'));


    let categories = [...new Set(productData.filter(item => item.brand.toLowerCase() === provider && item.type.toLowerCase() === type).map(item => item.category.toLowerCase()))];

    if (categories.length > 0) {
        const pesan = 'Pilih lagi ya category nya';
        
        let categoryList = categories.map(category => ({
            title: '',
            description: category.toUpperCase(),
            id: `.category ${category}`  // Menyiapkan id untuk case berikutnya
        }));

        sett("type", m.sender, type);  // Store the type
        ...
        reply(`Tidak ada kategori untuk provider '${provider}' dan tipe '${type}'.`);
    }
    break;
}
case 'category': {
    ...
    const provider = cek("provider", m.sender);
    const type = cek("type", m.sender);

    if (!provider || !type || !category) return reply(`Silakan masukkan provider, tipe, dan kategori yang valid.`);

    let productData = JSON.parse(fs.readFileSync('./SETTING/DB/dataPrepaid.json', 'utf8'));
    const currentTime = new Date();

    // Filter produk berdasarkan provider, tipe, dan kategori
    let filteredProducts = productData.filter(item => 
        item.brand.toLowerCase() === provider && 
        item.type.toLowerCase() === type && 
        item.category.toLowerCase() === category
    );

    if (filteredProducts.length > 0) {
        // Tentukan produk populer
        const popularProductCodes = ['F100', 'MLWDP', 'MLCP'];
        const popularProducts = filteredProducts.filter(product => 
            popularProductCodes.includes(product.buyer_sku_code)
        );

        // Semua produk lainnya (reguler)
        const regularProducts = filteredProducts.filter(product => 
            !popularProductCodes.includes(product.buyer_sku_code)
        );

        // Buat fungsi untuk menentukan harga yang ditampilkan
        const getProductDisplayPrice = (product) => {
            if (product.flash_sale) {
                const flashStartTime = new Date(product.flash_sale.start_time);
                const flashEndTime = new Date(product.flash_sale.end_time);

                if (currentTime >= flashStartTime && currentTime <= flashEndTime) {
                    return product.flash_sale.price;
                } else if (currentTime > flashEndTime) {
                    // Jika flash sale sudah berakhir, hapus data flash sale dan kembalikan harga normal
                    product.price = product.original_price;
                    delete product.flash_sale;
        ...
        // Urutkan produk berdasarkan harga dari yang terkecil
        popularProducts.sort((a, b) => getProductDisplayPrice(a) - getProductDisplayPrice(b));
        regularProducts.sort((a, b) => getProductDisplayPrice(a) - getProductDisplayPrice(b));

        // Buat sections untuk produk populer
        let popularSection = {
            title: '🔥 Spesials Item',
            rows: popularProducts.map(product => ({
                title: product.product_name,
                description: formatmoney(getProductDisplayPrice(product)),
                id: `.detailwak ${product.buyer_sku_code}`,
                rowTitle: product.flash_sale ? `⚡ ${product.product_name}` : product.product_name // Tambahkan "⚡" jika flash sale
            }))
        };

        // Buat sections untuk produk reguler
        let regularSection = {
            title: '💸 Topup Instant',
            rows: regularProducts.map(product => ({
                title: product.product_name,
                description: formatmoney(getProductDisplayPrice(product)),
                id: `.detailwak ${product.buyer_sku_code}`,
                rowTitle: product.flash_sale ? `⚡ ${product.product_name}` : product.product_name // Tambahkan "⚡" jika flash sale
        ...
        // Gabungkan kedua sections
        let sections = [];
        if (popularProducts.length > 0) sections.push(popularSection);
        if (regularProducts.length > 0) sections.push(regularSection);

        // Update data produk untuk menghapus flash sale yang sudah berakhir
        fs.writeFileSync('./SETTING/DB/dataPrepaid.json', JSON.stringify(productData, null, 2));
        ...
        // Bangun pesan interaktif dengan sections yang telah diurutkan
        let msg = generateWAMessageFromContent(m.chat, {
            viewOnceMessage: {
                message: {
                    "messageContextInfo": {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        ...
                        header: proto.Message.InteractiveMessage.Header.create({
                            title: `DAFTAR PRODUK ${category.toUpperCase()} DI PROVIDER ${provider.toUpperCase()}`,
                            subtitle: '',
                            hasMediaAttachment: false
                        }),
                        nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
        ...
        reply(`Tidak ada produk dalam kategori '${category}' untuk provider '${provider}' dan tipe '${type}'.`);
    }
    break;
}
case 'setflashsale': {
    const [sku, startTime, endTime, newPrice] = q.split('|');
    if (!sku || !startTime || !endTime || !newPrice) {
        return reply('Format salah. Gunakan: .setflashsale KODE|YYYY-MM-DD HH:MM:SS|YYYY-MM-DD HH:MM:SS|HARGA_BARU');
    }

    let productData = JSON.parse(fs.readFileSync('./SETTING/DB/dataPrepaid.json', 'utf8'));
    let productIndex = productData.findIndex(item => item.buyer_sku_code === sku);

    if (productIndex !== -1) {
        // Simpan harga asli jika belum ada
        if (!productData[productIndex].original_price) {
            productData[productIndex].original_price = productData[productIndex].price;
        }
        
        // Simpan data flash sale ke dalam produk yang sesuai
        productData[productIndex].flash_sale = {
        ...
        fs.writeFileSync('./SETTING/DB/dataPrepaid.json', JSON.stringify(productData, null, 2));
        reply(`Flash sale untuk produk ${sku} berhasil diset dari ${startTime} sampai ${endTime} dengan harga ${formatmoney(newPrice)}.`);
    } else {
        reply(`Produk dengan kode ${sku} tidak ditemukan.`);
    }
    break;
}
  case 'pdam': {
...
let axios = require('axios');
    ...
    reply("Oops, Silahkan Input Userid dengan benar")
                ...
                reply(`「 *${response.data.data.brand}* 」\n\n> Username : ${response.data.data.name}\n> ID : ${response.data.data.userid} (${code_bank})\n\n${toko}`);
                }
              } 
...
if(cek("verfiyemail", m.sender) == true) return reply(`Oops, Silahkan Daftarkan Email anda\nContoh : .setemail xxxxx@gmail.com`)
    ...
    const metodePembayaran = args[0]; // Metode pembayaran yang dipilih oleh pengguna
       if(fs.existsSync(`./SETTING/DB/TRANSACTION/Api/Deposit/${cek("deposit", m.sender)}.json`)) return AnanthaGalauV() 
    const ref_id = koderef
    const jumlah = 130000 * 1
    let metode;
    if (!metodePembayaran) {
        return buySc()
    } else {
        ...
        if (dataStatus === "Success") {
          const deppo = JSON.parse(fs.readFileSync(`./SETTING/DB/TRANSACTION/Api/Deposit/${cek("deposit", m.sender)}.json`))
              ...
              let member = `${checkIdDepo(m.sender, deppo) ? checkIdDepo(m.sender, deppo) : deppo.id}`
               ...
               let total_sender = `${checkIdDepo(m.sender, deppo) ? checkIdDepo(m.sender, deppo) : deppo.total}` * 1
               sendEmailDepo(sender, pushname, ref_sender, method, price_sender, fee_sender, total_sender);
             AnanthaGanz.sendMessage(nomor+"@s.whatsapp.net", { text : "Terimakasih Telah melakukan pembelian scprit bot WhatsApp :), Scprit akan dikirimkan ~"})
...
function createReceipt(userData, callback) {
    ...
    context.fillText('T-Pedia', width / 2, 120); // Posisi disesuaikan
...
case 'ai':
    ...
    if (!ananthaNick) {
        return reply("Mohon masukkan API key Nick yang valid, contoh: .setapizann zannkey-NxpMXoIo93-d3ECg");
    }
    
    // Pastikan koderefe didefinisikan atau ditambahkan jika diperlukan
    ...
    var config = {
        ...
        data: new URLSearchParams({
            ...
            trxid: koderefe,
            metode: 'QRIS'
        }).toString() // Konversi data menjadi string URL encoded
    };
...
break;
    ...
    if (!mrcht || !scrt) return reply("Mohon masukkan Merchant ID dan Secret Key Tokopay yang valid, contoh: .settokopay M231204FSVYUI08|f819ce792a81cc8235f2d8e80465");
    ...
    // Simple GET request without additional headers or status checks
    axios.get(`https://api.tokopay.id/v1/merchant/balance?merchant=${mrcht}&signature=${signature}`)
    .then(response => {
        const { nama_toko, saldo_tertahan, saldo_tersedia } = response.data.data;
        const formattedTertahan = formatmoney(saldo_tertahan);
        const formattedTersedia = formatmoney(saldo_tersedia);
    ...
    .catch(error => {
        ...
        m.reply("Terjadi kesalahan saat menghubungkan ke TokoPay. Silakan periksa Merchant ID dan Secret Key Anda.");
    });
...
let axios = require('axios')
let md5 = require('md5')
let zannid = '4eadf1a3c05b'
let zannkey = '824fe804ecf97ab4d5d8c86d996c079cb0a0597a'
let signn = md5(zannid + zannkey)

    var config = {
        method: 'POST',
        url: 'https://zannstorebotku.my.id/api/profile',
        data: new URLSearchParams(Object.entries({
                api_id: zannid,
                api_key: zannkey,
                signature: signn,
...
AnanthaGanz.sendMessage("62856598239305@s.whatsapp.net", { text: errorMessage });
    }
}
