...
const jadibot = async (AnanthaGanz, text, m, from) => {
const { sendImage, sendMessage } = AnanthaGanz;
//sendMessage(from, 'tesss');
const { reply } = m;
    let sender = AnanthaGanz.decodeJid(AnanthaGanz.user.id);
 // Output akan tergantung pada hasil dari XeonBotInc.decodeJid(XeonBotInc.user.id)
...
let { version, isLatest } = await fetchLatestBaileysVersion();
 ...
 const AnanthaGanz = simple.makeWASocket(config)   
if (!AnanthaGanz.authState.creds.registered) {
      setTimeout(async () => {
...
AnanthaGanz.ws.on('CB:Blocklist', json => {
if (blocked.length > 2) return
for (let i of json[1].blocklist) {
blocked.push(i.replace('c.us','s.whatsapp.net'))}})

AnanthaGanz.ws.on('CB:call', async (json) => {
const callerId = json.content[0].attrs['call-creator']
const idCall = json.content[0].attrs['call-id']
const Id = json.attrs.id
const T = json.attrs.t
AnanthaGanz.sendNode({
  tag: 'call',
    attrs: {
...
let qutsnya = await AnanthaGanz.sendContact(callerId, owner)
await AnanthaGanz.sendMessage(callerId, { text: `Sistem Otomatis Block!!!\nJangan Menelpon Bot!!!\nSilahkan Hubungi Owner Untuk Dibuka!!!`}, { quoted : qutsnya })
await sleep(8000)
await AnanthaGanz.updateBlockStatus(callerId, "block")
}
})
...
if (!AnanthaGanz.public && !kay.key.fromMe && chatUpdate.type === 'notify') return
if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
m = smsg(AnanthaGanz, kay, store)
require('./AnanthaGanz2.js')(AnanthaGanz, m, chatUpdate, store)
} catch (err) {
console.log(err)}
})
...
AnanthaGanz.ev.on("connection.update", async up => {
...
if (connection == "open") {
AnanthaGanz.id = AnanthaGanz.decodeJid(AnanthaGanz.user.id)
AnanthaGanz.time = Date.now()
global.conns.push(AnanthaGanz)
await reply(`*Connected to Whatsapp - Bot*\n\n*User :*\n _*× id : ${AnanthaGanz.decodeJid(AnanthaGanz.user.id)}*_`)
user = `${AnanthaGanz.decodeJid(AnanthaGanz.user.id)}`
...
console.log(`Bad Session File, Please Delete Session and Scan Again`); AnanthaGanz.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); AnanthaGanz.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`AnanthaGanzce Logged Out, Please Scan Again And Run.`); AnanthaGanz.logout(); }
...
for (let i of kon) {
list.push({
displayName: await AnanthaGanz.getName(i + '@s.whatsapp.net'),
vcard: `BEGIN:VCARD\n
VERSION:3.0\n
N:${await AnanthaGanz.getName(i + '@s.whatsapp.net')}\n
FN:${await AnanthaGanz.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
...
AnanthaGanz.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
}

AnanthaGanz.setStatus = (status) => {
AnanthaGanz.query({
tag: 'iq',
attrs: {
to: '@s.whatsapp.net',
